...
def mapPartition(layer1,layer2, xbar_length, LayerNUM,hpar,vpar,metal,T,H,L,W,D,eps,rho,weight_var,data_dir,spice_dir): 
    ...
    for x_id in range(hpar):
        for y_id in range(vpar):
            ...
            for split_vpar in range(xbar_length):
                ...
                fd.write(".SUBCKT layer"+ str(LayerNUM)+" vdd vss 0 ")
...
mapPartition(120, 84, 32, 2, 4, 3, 2.69999999997e-8, 2.2e-8, 2e-8, 1.35e-7, 1.08e-7, 4.5e-8, 1.77079999999e-10, 1.9e-8, 0, 'data', 'test_spice')