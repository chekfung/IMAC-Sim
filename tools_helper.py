...
# # Read Simulation File. Defaults to Cadence Virtuoso, but 
# def read_simulation_file(sp_filepath, simulator='spectre'):
#     """
#     Reads the simulation output file based on the specified simulator.
...
#     Returns:
#         object: A simulation file object (PSF for Spectre or RawRead for LTSpice).
...
#     elif sim_str == 'hspice':
#         # Make sure that end is .psf
#         basename = os.path.basename(raw_fp)
#         assert(basename.rsplit('.', 1)[1] == 'psf')

#         # Use PSF utils to open the raw file :)
#         psf = PSF(raw_fp)
#         return psf
...
def print_signal_names(sim_file_obj, simulator='spectre'):
    ...
    if sim_str == 'spectre' or sim_str == 'hspice':
        ...
        # LTSpice Sim .RAW file
        # FIXME: Using LTSpice does not return the correct trace names right now
...
def get_signal(signal_name, sim_file_obj, simulator='spectre'):
    ...
    Raises:
        ValueError: If the simulator is not supported.
    """
    # FIXME: With HSPICE, no voltage v() in front of names and refers to the net name. With spectre, they have the v connected to the net
    
    # Make sure simulator string is not something crazy
    # FIXME: Tentatively, no error checking on signal names 
    sim_str = idiot_proof_sim_string(simulator)
    assert(is_simulator_real(sim_str))
...
def run_synopsys_hspice(args):
    ...
    FIXME: In the future, we should use a Synopsys tool, but this is a nice band-aid fix for things for now
...
def write_sim_specific_simulation_information(fp, NUMBER_OF_INPUTS, probe_all_nets=True, voltage_nets=[],current_nets=[],simulator='spectre'):
    ...
    if sim_str == 'spectre':
        ...
        #TODO: Needs to be standardized such that we only probe the things that we want.
        ...
        # write_line_with_newline(fp, '.PROBE v(v0) v(v1) v(v2) v(v3) v(v4) v(v5) v(v6) v(v7) v(v8) v(v9) v(v10) v(v11) v(v12) v(v13) v(v14) v(v15) v(v16) v(v17) v(v18) v(v19) v(v20) v(v21) v(v22) v(v23) v(v24) v(v25) v(v26) v(v27) v(v28) v(v29) v(v30) v(v31) v(out1)')
        write_line_with_newline(fp, '.PROBE v(vdd) v(vss)')
        #write_line_with_newline(fp, '.MEASURE TRAN vout_5ns FIND v(out1) AT=5ns')
...
def change_SPICE_param_in_file(infile, outfile, objects_to_modify):
    """
